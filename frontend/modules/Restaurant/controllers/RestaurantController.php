...
class RestaurantController extends CommonController
{
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['restaurant-service','food-service','providereason','active','deactive','pauserestaurant','resume-restaurant','cooking-detail','phonecooking','orderlist','food-on-off','selectionactive','record-restaurant-name','todayinvoicepdf'],
                        'allow' => true,
    ...
    public function actionTodayinvoicepdf()
    {   
        $ids = [];
        $ids[] = 1;
        $ids[] = 8;
        ...
        $order = Orders::find()->where('Orders_Date = :did', [':did'=>date('Y-m-d')])->andWhere(['not in', 'Orders_Status',$ids])->all();
        $countorder = Orders::find()->where('Orders_Date = :did', [':did'=>date('Y-m-d')])->andWhere(['not in', 'Orders_Status',$ids])->count();
        

        if(empty($order))
        {
            Yii::$app->session->setFlash('error', Yii::t('cart','No order today!'));
            return $this->redirect(['/Restaurant/restaurant/restaurant-service']);
        }   
         foreach ($order as $kk => $did) {
            $orderitem = Orderitem::find()->where('Delivery_ID = :did',[':did'=>$did['Delivery_ID']])->andWhere(['not in', 'OrderItem_Status',$ids])->all();  
        }
        $address = DeliveryAddress::find()->where('delivery_id=:did',[':did'=>$did['Delivery_ID']])->one();
        $nicknames = array();
        ...
        $pdf = new Pdf([
            'mode' => Pdf::MODE_UTF8,
            'content' => $this->renderPartial('todayinvoicepdf',['order'=>$order, 'orderitem' => $orderitem ,'address'=>$address,'nicknames'=>$nicknames]),
            'options' => [
                'title' => 'Invoice',
